...
class Orders extends CI_Controller {
	...
	function edit($id=-1,$error='',$values=NULL) {
		if ((!$this->my_application->hasPermission("Orders","Insert") && $id <= 0) || 
			...
			if (isset($values) && isset($values['details'])) $order['details'] = $values['details'];
			...
			if ($userRol == "" && (float)$order['userId'] == $this->session->userdata('userId')) {
		  	...
		  	$allowAFullCancel = ($order["stateId"] == "TOAUT" && $this->my_application->hasPermission("Orders","FullCancel"));			  			  	
		  	if ($allowAFullCancel) {
		  		$onlyCurrentState = false;
			...
			if (count($contentData['states']) > 0 && $userRol != "") {				
		  	...
		  	if ($allowEditByState && (($this->my_application->hasPermission("Orders","Insert") && $id <= 0) || 
		  		...
		  		switch ($userRol) {
		  			case "manager":
		  				if (($order["stateId"] == "PROG" || $order["stateId"] == "BUDGET") && (float)$order['managerId'] == $this->session->userdata('userId')) {
		  	...
		  	$contentData['allowSave'] = ($contentData['allowSave'] || $contentData['allowEditGeneralData'] || $contentData['allowEditDetail'] || $contentData['allowEditDetailImports']);					  	
			...
			if (!$contentData['allowSave'] && $order["stateId"] != "FIN" && $order["stateId"] != "CAN" && $order["stateId"] != "SUS" && $order["stateId"] != "NOTAUT") {
		  	...
		  	if (!$contentData['allowEditGeneralData']) {
			...
			/*
			$allowSelectCompany = true;
			if ($order['id'] <= 0 && $allowSelectCompany) {				
			...
			*/
			...
			if ((float)$order['id'] <= 0 && $order['stateId'] == "TOAUT" && (float)$order['userId'] == $this->session->userdata('userId')) {
	...
	function save() {   		
		if ((!$this->my_application->hasPermission("Orders","Edit") && $this->input->post('id',TRUE) > 0) || 
			...
			$this->form_validation->set_rules('userEmail','Fecha','trim|xss_clean');
			$this->form_validation->set_rules('userCellphone','Fecha','trim|xss_clean');
			...
			$this->form_validation->set_rules('detailAmount','Subttotal','trim|xss_clean');
			$this->form_validation->set_rules('ivaRate','Porc. IVA','trim|xss_clean');
			$this->form_validation->set_rules('ivaAmount','IVA','trim|xss_clean');
	...
	function pdf($id=0,$documentType='') {		
		if ((float)$id > 0 && trim($documentType) == 'PO' && 
			$this->session->userdata('userLoggedIn') && 
	...
	function excel($id=0,$type='') {											
		...
		if ($showImports) {
			$row++;
			$col = $colsCount - 2;
			$sheet->setCellValue(getLetterOfExcelColumn($col).$row,"Subtotal");  
			$sheet->mergeCells(getLetterOfExcelColumn($col).$row.':'.getLetterOfExcelColumn($col+1).$row);
			$styleCell = array(
					            'fill' => array(
					                'fillType' => \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID,
					                'startColor' => array('argb' => '007E7E80')
					            	),
					            'font' => array(
					            	'bold'=>true,
					            	'color' => array('argb' => 'FFFFFFFF')
			...
			$col = $colsCount;
			$sheet->setCellValue(getLetterOfExcelColumn($col).$row,decimalFormat($order['detailAmountWithIVA'],2));  
			$styleCell = array(
					            'fill' => array(
					                'fillType' => \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID,
					                'startColor' => array('argb' => '00F4F6F9')
			...
			$styleCell = array(
					            'fill' => array(
					                'fillType' => \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID,
					                'startColor' => array('argb' => '00A8A9AB')
					            	),
					            'font' => array(
					            	'bold'=>true,
					            	'color' => array('argb' => 'FFFFFFFF')
			...
			$sheet->getStyle(getLetterOfExcelColumn($col).$row.":".getLetterOfExcelColumn($col+1).$row)->applyFromArray($styleCell);		
			$col = $colsCount;
			$sheet->setCellValue(getLetterOfExcelColumn($col).$row,decimalFormat($order['detailAmountWithIVA']-$order['detailAmount'],2));  
			$styleCell = array(
					            'fill' => array(
					                'fillType' => \PhpOffice\PhpSpreadsheet\Style\Fill::FILL_SOLID,
					                'startColor' => array('argb' => '00F4F6F9')
	...
	function deliveryNote($orderId=-1,$deliveryNoteId=-1) {
		if (!$this->my_application->hasPermission("Orders","See") || $orderId <= 0) {
			...
			if ($deliveryNoteId > 0) {
				...
				$contentData['allowSave'] = ($this->my_application->hasPermission("Orders","GenerateDN") && isset($contentData['details']) && count($contentData['details']) > 0);				
				$contentData['allowPrint'] = false;
				$contentData['showReceivedData'] = false;	
			}
